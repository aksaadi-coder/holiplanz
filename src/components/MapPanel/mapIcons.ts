...
export function numberedIcon(number: number, highlighted: boolean, flagged: boolean): L.DivIcon {
  const size = highlighted ? 34 : 28;
  const border = flagged ? "dashed 2px #666" : "solid 2px white";
  return L.divIcon({
    className: "stop-marker",
    html: `<div class="stop-marker-inner" style="width:${size}px;height:${size}px;background:${
      flagged ? "#999" : MARKER_COLOR
    };border:${border};">${number}</div>`,
    iconSize: [size, size],
