...
import type { SpotlightFeature } from "./FeatureSpotlight/FeatureSpotlight";
...
const FEATURES: { key: SpotlightFeature; icon: ReactElement; title: string; description: string }[] = [
  {
    key: "itinerary",
    icon: (
  ...
  },
  {
    ...
    description: "Chat changes like “make day 2 more relaxed” - or drag, swipe and check off stops by hand.",
  },
  {
    key: "save",
    icon: (
...
export function TripSetupForm({
  ...
  const [spotlight, setSpotlight] = useState<SpotlightFeature | null>(null);
  const destinationRef = useRef<HTMLInputElement>(null);

  function handleSpotlightPlanTrip() {
    setSpotlight(null);
    ...
    <div className="landing-page">
      ...
      <div className="feature-grid">
        {FEATURES.map((feature) => (
          <button
            ...
            key={feature.title}
            onClick={() => setSpotlight(feature.key)}
          >
            <svg
            ...
            <span className="feature-card-hint">See it in action &rarr;</span>
          </button>
        ))}
      </div>
      ...
      {spotlight && (
        <FeatureSpotlight
          ...
          onPlanTrip={handleSpotlightPlanTrip}
        />
      )}
